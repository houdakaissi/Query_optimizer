...
def extract_text(image_path):
    ...
    return {'message': result_message, 'action': action}
...
def index():
    ...
    cars_data = [{'id': car.id, 'matricule': car.matricule, 'model': car.model} for car in cars]
    return jsonify({'users': users_data, 'cars': cars_data})
...
def subscribe():
    ...
    if not all([username, matricule, model, start_date, start_time, end_date, end_time]):
    ...
    if existing_subscription:
        return jsonify({'error': 'A subscription with the same matricule already exists.'}), 400
...
@app.route('/subscribe')
def subscribtio():
    return jsonify({'message': 'This endpoint is used for subscription purposes.'})
'''
...
def capture_image():
    ...
    if ret:
        ...
        return jsonify({"error": "Failed to capture and save image"}), 500
